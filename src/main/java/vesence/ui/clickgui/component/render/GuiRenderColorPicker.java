...
public class GuiRenderColorPicker extends GuiScreen {
   ...
   private static void renderColorPicker(
      ...
      float pickerX = x + (30.0F - 30.0F * (float)GuiScreen.animation15.getOutput());
      float pickerWidth = width;
      float pickerHeight = height - 5.0F;

      renderer2D.rectOutline(pickerX, y, pickerWidth, pickerHeight, 5.5F, outlineColor, 0.1F);
      renderer2D.rect(pickerX, y, pickerWidth, pickerHeight, 5.5F, bgColor);

      float paletteWidth = 63.92F;
      float paletteHeight = 47.02F;
      float paletteX = x + 5.0F + (30.0F - 30.0F * (float)GuiScreen.animation15.getOutput());
      ...
      float hueSliderX = x + 5.0F + (30.0F - 30.0F * (float)GuiScreen.animation15.getOutput());
   ...
   private static void renderHueSlider(Renderer2D renderer2D, float x, float y, float width, float height) {
      ...
      for (int i = 0; i < segments; i++) {
         ...
         renderer2D.horizontalGradient(
            ...
            segmentWidth,
            height,
            ColorUtil.multAlpha(hueColors[i].getRGB(), (float)GuiScreen.animation15.getOutput()),
            ColorUtil.multAlpha(hueColors[i + 1].getRGB(), (float)GuiScreen.animation15.getOutput())
         );
      }
   }
}

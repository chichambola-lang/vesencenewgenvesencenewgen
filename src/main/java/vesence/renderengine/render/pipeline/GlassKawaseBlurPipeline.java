...
public class GlassKawaseBlurPipeline {
    private static final Identifier DOWN_PIPELINE_ID = Identifier.of("vesence", "pipeline/glass_kawase_down");
    private static final Identifier DOWN_VERTEX_SHADER = Identifier.of("vesence", "core/glass_kawase_down");
    private static final Identifier DOWN_FRAGMENT_SHADER = Identifier.of("vesence", "core/glass_kawase_down");
    private static final Identifier UP_PIPELINE_ID = Identifier.of("vesence", "pipeline/glass_kawase_up");
    private static final Identifier UP_VERTEX_SHADER = Identifier.of("vesence", "core/glass_kawase_up");
    private static final Identifier UP_FRAGMENT_SHADER = Identifier.of("vesence", "core/glass_kawase_up");
    ...
    private void ensureInitialized() {
        ...
        this.dummyVertexBuffer = RenderSystem.getDevice().createBuffer(
            () -> "vesence:glass_kawase_dummy_vertex",
    ...
    private void ensureFramebuffers(int width, int height) {
        ...
        for (int i = 0; i < MAX_ITERATIONS; i++) {
            ...
            downWidths[i] = currentWidth;
            downHeights[i] = currentHeight;
            upWidths[i] = currentWidth;
            upHeights[i] = currentHeight;

            downTextures[i] = RenderSystem.getDevice().createTexture(
                () -> "vesence:glass_kawase_down_" + finalIndex,
                GpuTexture.USAGE_COPY_DST | GpuTexture.USAGE_TEXTURE_BINDING | GpuTexture.USAGE_RENDER_ATTACHMENT,
                TextureFormat.RGBA8,
                currentWidth,
                currentHeight,
            ...
            upTextures[i] = RenderSystem.getDevice().createTexture(
                () -> "vesence:glass_kawase_up_" + finalIndex,
                GpuTexture.USAGE_COPY_DST | GpuTexture.USAGE_TEXTURE_BINDING | GpuTexture.USAGE_RENDER_ATTACHMENT,
                TextureFormat.RGBA8,
                currentWidth,
                currentHeight,
    ...
    public GpuTextureView blur(GpuTexture sourceTexture, GpuTextureView sourceView, int width, int height, int iterations, float offset) {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            try (RenderPass renderPass = encoder.createRenderPass(() -> "vesence:glass_kawase_down_pass_" + finalIndex, downTextureViews[i], OptionalInt.empty())) {
        ...
        for (int i = iterations - 1; i >= 0; i--) {
            ...
            try (RenderPass renderPass = encoder.createRenderPass(() -> "vesence:glass_kawase_up_pass_" + finalIndex, upTextureViews[i], OptionalInt.empty())) {
        ...
        try (RenderPass renderPass = encoder.createRenderPass(() -> "vesence:glass_kawase_final_pass", finalTextureView, OptionalInt.empty())) {
            renderPass.setPipeline(UP_PIPELINE);
            renderPass.setVertexBuffer(0, dummyVertexBuffer);
            renderPass.bindTexture("Sampler0", currentSource, sampler);
            RenderSystem.bindDefaultUniforms(renderPass);

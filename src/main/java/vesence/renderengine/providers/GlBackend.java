package vesence.renderengine.providers;
...
import vesence.renderengine.render.RenderFrameMetrics;
import vesence.renderengine.postfx.DepthRenderTarget;
...
public final class GlBackend {
   ...
   private final Set<Integer> nearestTextures = new HashSet<>();
   ...
   public GlBackend() {
      ...
      if (this.ssboSupported || instancedArraysSupported && drawInstancedSupported) {
         String vertexShaderPath = this.ssboSupported ? "assets/vesence/shaders/shape.vert"
               : "assets/vesence/shaders/shape_compat.vert";
         this.shapeProgram = ShaderProgram.fromResources(vertexShaderPath, "assets/vesence/shaders/shape.frag");
   ...
   private ShaderProgram ensureFullscreenProgram() {
      if (this.fullscreenProgram != null) {
         ...
         String vertex = ResourceUtils.readText("assets/vesence/shaders/blur/blur_fullscreen.vert");
   ...
   public void flush() {
      if (this.instanceCount > 0) {
         ...
         for (int slot = 0; slot < usedSlots; slot++) {
            ...
            if (this.nearestTextures.contains(tex)) {
         ...
         this.textureToSlot.clear();
         this.slotToTexture.clear();
         this.nearestTextures.clear();
      }
   }

   public void useNearestFilter(int texture) {
      if (texture > 0) {
         this.nearestTextures.add(texture);
      }
   }
   ...
   private void writeInstanceEx(
      ...
      if (this.instanceCount >= 4096) {
         ...
         float pad = (type != 3) ? 2.0F : 0.0F;
         putVertices(this.instanceBuffer, transform, x - pad, y - pad, w + pad * 2.0F, h + pad * 2.0F);
   ...
   public void enqueueGradientOutline(
         float x,
         float y,
         float w,
         float h,
   ...
   public void enqueueCircleOutline(float cx, float cy, float radius, float startDeg, float pct, int color,
      ...
      float expanded = radius + thickness;
      float size = expanded * 2.0F;
      this.ensureInstanceCapacity();
      this.writeInstanceEx(2, cx - expanded, cy - expanded, size, size, color, color, color, color,
            radius, 0.0F, 0.0F, 0.0F, thickness, transform, 0.0F, 0.0F, 1.0F, 1.0F, -1, startDeg, pct, 0);
   }
   ...
   public void enqueueMsdfGlyph(
         ...
         float v1, int rgbaColor, float[] transform, float fontWeight) {
      if (texture > 0) {
         this.ensureInstanceCapacity();
         int slot = this.textureSlotFor(texture);
         float clampedRange = pxRange > 0.0F ? pxRange : 0.001F;

         int weightEncoded = Math.max(0, Math.min(31, Math.round(fontWeight * 16.0f + 16.0f)));
         int weightFlags = weightEncoded << 27;
         this.writeInstanceEx(
               3,
               x,
               y,
   ...
   public void enqueueMsdfGlyphMultiColor(
         int texture, float pxRange, float x, float y, float width, float height, float u0, float v0, float u1,
         float v1, int colorTL, int colorTR, int colorBR, int colorBL, float[] transform, float fontWeight) {
      if (texture > 0) {
         this.ensureInstanceCapacity();
         int slot = this.textureSlotFor(texture);
         float clampedRange = pxRange > 0.0F ? pxRange : 0.001F;
   ...
   public GlBackend.FrameCapture captureFullFrame() {
      ...
      if (client == null) {
         ...
         if (framebuffer == null) {
            return new GlBackend.FrameCapture(0, 0, 0, 0);
         } else if (extractGlTexId(framebuffer) == 0) {
            return new GlBackend.FrameCapture(0, 0, 0, 0);
         } else {
            int sourceColor = extractGlTexId(framebuffer);
            int sourceDepth = 0;
   ...
   private static boolean extractLogDone = false;
   private static boolean blurLogDone = false;
   ...
   private static int extractGlTexId(Framebuffer framebuffer) {
      ...
      try {
         ...
         if (view != null && !view.isClosed()) {
            ...
            if (!extractLogDone) {
               System.out.println("[VesenceBlur] view=" + view + " tex=" + tex + " isGlTexture=" + (tex instanceof GlTexture));
               extractLogDone = true;
            }
            if (tex instanceof GlTexture glTex) return glTex.getGlId();
         }
      } catch (Exception e) {
         if (!extractLogDone) { System.out.println("[VesenceBlur] view exception: " + e); extractLogDone = true; }
      }

      try {
         if (framebuffer.getColorAttachment() instanceof GlTexture glTex) return glTex.getGlId();
      } catch (Exception ignored) {}
      if (!extractLogDone) { System.out.println("[VesenceBlur] both paths returned 0"); extractLogDone = true; }
      return 0;
   }
}

...
vec4 aurora(vec3 ro, vec3 rd)
{
    ...
    {
        float fi = float(i);
        float of = 0.006*hash21(vLocalPx)*smoothstep(0.,15., fi);
        float pt = ((.8+pow(fi,1.4)*.002)-ro.y)/(rd.y*2.+0.4);
        pt -= of;
        vec3 bpos = ro + pt*rd;
        vec2 p = bpos.zx;
        float rzt = triNoise2d(p, 0.06);
        vec4 col2 = vec4(0,0,0, rzt);
        col2.rgb = (sin(1.-vec3(2.15,-.5, 1.2)+fi*0.043)*0.5+0.5)*rzt;
        avgCol =  mix(avgCol, col2, .5);
        col += avgCol*exp2(-fi*0.065 - 2.5)*smoothstep(0.,5., fi);
    }
...
vec3 orbMain()
{
    ...
    vec2 q = vLocalPx / vSize;
    vec2 p = q - 0.5;
    p.x *= vSize.x / vSize.y;

    vec3 ro = vec3(0,0,-6.7);
    vec3 rd = normalize(vec3(p,1.3));
    vec2 mo = iMouse / vSize - 0.5;
    mo = (mo==vec2(-.5))?mo=vec2(-0.1,0.1):mo;
    mo.x *= vSize.x / vSize.y;
    rd.yz *= mm2(mo.y);
